...
function FilterSelect({ options }) {
  ...
  const handleFilterChange = (event) => {
    const filterValue = options[selected].address;
    const searchParams = new URLSearchParams(location.search);
    searchParams.set('complex', filterValue);
    console.log(searchParams.toString());
    //const navigate = useNavigate();
    //navigate(`/landlord-application?${searchParams.toString()}`);
  };
    ...
    <div className="relative">
      ...
      >
        <span className="flex items-center">
          ...
          <span>{options && options[selected]?.address}</span>
        </span>
            ...
            options && options.map((option, index) => {
              return (
                <button
                  key={option._id}
                  tabIndex="0"
                  className={`flex items-center w-full hover:bg-slate-50 hover:dark:bg-slate-700/20 py-1 px-3 cursor-pointer ${option._id === selected && 'text-indigo-500'}`}
                  onClick={(e) => { setSelected(index); setDropdownOpen(false); handleFilterChange(e); }}
                >
                  <svg className={`shrink-0 mr-2 fill-current text-indigo-500 ${option._id !== selected && 'invisible'}`} width="12" height="9" viewBox="0 0 12 9">
